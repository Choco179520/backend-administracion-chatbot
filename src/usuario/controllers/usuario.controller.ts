...
import { AdminAccess, PublicAccess, Roles } from "src/common/decorators/decoradores-aplicativo.decorators";
...
export class UsuarioController {
    constructor(
     ...
     @ApiOperation({description: 'Obtener informacion de usuarios mediante params'})
     @Get('find-params')
     async findParams(
         @Query() criterioBusqueda: FilterDto
     ) {
         if (Object.keys(criterioBusqueda).length !== 0) {
             return await this._usuarioService.buscarTodos(
                 criterioBusqueda,
     ...
     @ApiOperation({description: 'Actualizar informacion del usuario en base de datos'})
     @Roles('ADMIN')
     @Put('update/:id')
